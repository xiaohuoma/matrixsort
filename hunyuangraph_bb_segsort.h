...
#ifndef _H_BB_SEGSORT_ME
#define _H_BB_SEGSORT_ME
...
int hunyuangraph_segmengtsort(K *keys_d, T *vals_d, int n,  int *d_segs, int length, int *d_bin_counter, int *d_bin_segs_id, K *keysB_d, T *valsB_d)
{
    ...
    // printf("subwarp_num=%d length=%d h_bin_counter[12]=%d",length-h_bin_counter[12],length,h_bin_counter[12]);
    if(subwarp_num > 0)
